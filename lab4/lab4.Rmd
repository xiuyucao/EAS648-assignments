...
library(reshape2)  # for get a matrix from data frame to plot comparison cloud
```
...
### Word Clouds
Here I use word clouds to visualize the frequency of words in the book. Here using the `with()` function, I will be able to directly call the attribute without doing a `$`.
```{r plot word cloud}
book_words %>%
...
Also, Lord Darlington plays a role in facilitating political discussions that ultimately serve the interests of Nazi Germany. His actions, driven by a misguided sense of honor and a desire to prevent another war, lead him down a path of appeasement towards Nazi Germany. While discussing Lord Darlington, Stevens consistently portrays him as a decent and kind man, offering unwavering praise for his character. However, readers can discern subtle criticisms embedded in the text regarding Lord Darlington's political involvement and his problematic standpoint. 

These ambiguity adds difficulty to sentiment analysis

Here, I want to analyze, as Steven says, the 'turning point' in his career. This moment occurred when his father fell seriously ill. Despite the gravity of the situation, Stevens did not spend time companying his father due to an ongoing important international meeting at Darlington Hall. Although he was profoundly saddened during this time, he restrained himself and continued to serve the guests of Lord Darlington, maintaining the 'dignity' expected of a butler.

In his narration of this night, Stevens does not directly comment on his grief; he even expresses pride. However, as readers, we can sense the underlying self-doubt and sadness in his narrative.
...
Using bing
```{r ambiguous analysis using bing}
amb_bing <- amb_words %>%
  inner_join(get_sentiments("bing")) %>%
  count(Chapter, sentence_id, sentiment) %>%
  spread(sentiment, n, fill = 0) %>%
...
# plot the result
ggplot(amb_bing, aes(sentence_id, sentiment, fill = Chapter)) +
  geom_col(show.legend = FALSE)
```
