...
In the following paragraph, however, Stevens does not directly comment on his grief; he even expresses pride. As readers though, we can sense the underlying self-doubt and sadness in his narrative.
...
ggplot(amb_nrc, aes(sentence_id, sentiment, fill = Chapter)) +
  geom_col(show.legend = FALSE)

emo_col <- names(amb_nrc)[4:ncol(amb_nrc)-1]  # get the sentiments in the paragraph
test <- select(amb_nrc, all_of(emo_col))
test <- pivot_longer(test, cols = everything(), names_to = "emotion", values_to = "count")
test <- filter(test, count!=0)
ungroup(test)
test <- group_by(test,emotion)
test <- count(test,emotion)
with(test, wordcloud(emotion, n, min.freq=0,max.word=10))

test %>%
  ggplot(aes(x=emotion,y=n))+  # set X and Y values
...
While sentiment analysis has proven invaluable, it is essential to acknowledge its shortcomings and challenges. When analyzing ambiguous text, although both unigram-based and sentence-based methods discerned the overall emotion in the text, they still have the following drawbacks:
...
* Irony and Sarcasm: These rhetorical devices involve saying one thing while meaning the opposite. Failure to recognize such nuances can lead to misinterpretations and undermine the reliability of sentiment analysis. For instance, when Stevens mentioned a 'large sense of triumph,' it may, from the author's or reader's perspective, be a form of sarcasm.

In summary, sentiment analysis is a complex task, especially when handling ambiguous text. Combining different approaches with other NLP techniques and machine learning models to accommodate the diversity and complexity of fuzzy text can lead to a more accurate result.
