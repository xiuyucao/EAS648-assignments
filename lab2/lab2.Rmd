...
******************************************************
## Get LiDAR data
My LiDAR data used in this case is downloaded on [OpenTopography](https://opentopography.org). I want to study the trees in [Forest Hill Cemetery](https://foresthillcemeteryaa.org) in Ann Arbor. This is a screenshot of my research area.
...
In this plot, there are points within regions I am not interested in. So I will use the function `lidR::clip_rectangle()` to clip the point clouds. The function `range()` can be used to decide the range of the coordinates of the LiDAR data. For example, `range(fhc_las$X)``
...
As is shown in the plot, ground points have been correctly classified.
...
dtm_kriging <- rasterize_terrain(fhc_aoi, algorithm=kriging(k=20))  # get DTM
dtm_prod <- terrain(dtm_kriging, v = c("slope", "aspect"), unit = "radians")  # get terrain characteristics
dtm_hillshade <- shade(slope = dtm_prod$slope, aspect = dtm_prod$aspect)  # compute hill shade
plot(dtm_hillshade, col =gray(0:30/30), legend =T, main='Digital Terrain Model (m)')  # plot shaded DTM
```
...
chm <- rasterize_canopy(nlas, res = 0.5, 
                        pitfree(thresholds = c(0, 5, 10, 15, 20),  # height thresholds
                                max_edge = c(0, 1)))  # max edge length of a triangle in the Delaunay triangulation
# plot CHM
plot(chm,col=height.colors(25),main='Canopy Height Model (m)')
```
...
```{r tree detection using fix-sized window (code to show), eval=F}
...
```{r tree detection using fix-sized window, echo=F}
ttops <- locate_trees(nlas, lmf(ws = 10))  # tree detection using a fixed window size
plot(chm, col = height.colors(50))  # plot CHM
plot(sf::st_geometry(ttops), add = TRUE, pch = 3)  # add tree centers to the CHM
```
...
While the sizes of tree canopy can vary based on the ages of trees, it is better to use a variable size window to detect individual trees. Here I developed a simple linear relationship. If it is for research purposes, a more sophisticated model is needed.
...
head(crowns)
plot(crowns["convhull_area"], main = "Crown area (convex hull)")  # plot tree segmentation result
...
******************************************************
## Applications
Using the tree metrics, we can select the trees and do tree based inventory. One interesting [application](https://www.sciencedirect.com/science/article/pii/S0034425723003711?casa_token=JhtyRO16F9cAAAAA:9kyyjwLegDiCe0d0-KCMeDeiZ954qH27Z3BbVqgCu5x3aThWzsFkxK-topS5hx5QJ2X24tcAIA) 
I recently read is using the tree metrics to get microclimate from macrocliamte, since trees have a cooling effect.

For example, I have a land surface temperature raster data which can be computed from Landsat imagery. Here I generate a raster data from CHM to represent the temprature data which can be calculated from Landsat imagery.
```{r}
temp=setValues(chm,20)
```
Then, calculate the metrics.
```{r}
library(tidyr)
ce_m <- crown_metrics(las_seg, ~list(zmax=max(Z), imean=mean(Intensity)))
ce_m$cooling_effect <- 0.7*ce_m$zmax + 0.1*ce_m$imean
r <- rast(ext(las_seg),resolution=10)
v <- vect(ce_m["cooling_effect"])
map <- terra::rasterize(v, r, field = "cooling_effect", fun = mean) # extract sum of G at 15m
plot(map, col = hcl.colors(15)) # some plotting

test <- resample(chm,map,method='bilinear')
plot(test)
...
Finally ,calculate the microlimate based on the relationship.
